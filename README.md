...
- Sparse MoE:
  - `Qwen/Qwen3-30B-A3B` — empirically validated GPU baseline; also validated on NVIDIA H200 (float16, N=63 L45=0.832 L46=0.916)
...
For command examples, note that `openai/gpt-oss-20b` should currently be run with `--quantize none`, not the BitsAndBytes `4bit` path used in the other example commands. The script automatically detects the MXFP4 quantization config and loads in bfloat16 to match the dequantized weight dtype.

**Currently verified GPU manual-reference outcomes**
- `openai/gpt-oss-20b` with `--quantize none --exit-layers 21 22` on NVIDIA H200 (bfloat16): sane trace; N=63 raw-exit exact match is `0.808` at `L21` and `0.879` at `L22`; L22 penultimate advantage confirmed. The model ships MXFP4-quantized weights that transformers dequantizes to bfloat16; the script automatically detects this and matches the dtype. This is the canonical sparse supporting evidence on this GPU path.
...
**Cross-runtime note on quality cliffs:** The canonical MLX Qwen3 result shows a sharp cliff from L46 (0.837) to L45 (0.463). On the GPU path, the same layers show a much milder drop: L46=0.916 to L45=0.832. Cliff probes extending one layer further confirm monotonic degradation: GPT-OSS L20=0.589→L21=0.808→L22=0.879; Qwen3 L44=0.793→L45=0.832→L46=0.916. The penultimate advantage is structural (holds on both runtimes), but the cliff severity appears runtime-sensitive. The layer ordering also survives extension from 48 to 64 generated tokens: GPT-OSS numbers change by less than 0.005; Qwen3 shows a uniform ~2–3% drop but preserves the monotonic L44 < L45 < L46 structure.

**Multi-oracle analysis:** Among tested oracle-style acceptance diagnostics, plain penultimate `top1_agree` yields the highest acceptance rate on both models without reducing composed quality under the manual-reference verifier path. Adding entropy gating at τ=1.5 reduces GPT-OSS L22 acceptance from 87.8% to 71.0% while barely affecting Qwen3 L46 (91.6%→90.5%); margin gating (τ=0.1) and two-layer agreement also lower acceptance without demonstrated quality benefit. Oracle design is model-sensitive.

**Stage B karma logistic (GPU, verifier path):** Multi-feature karma classifiers achieve 0.859 precision / 0.085 error on GPT-OSS and 0.864 precision / 0.078 error on Qwen3, materially outperforming entropy-only baselines (0.564 / 0.529 precision). Token-row triage under shared full-depth context: 81–83% of tokens match full depth at earlier exit, 9–10% additionally match at penultimate, 8–9% require full depth. These are offline verifier diagnostics, not deployment routing decisions.
